...
class FrontController {
	...
	{
		$er = (self::config('production') ? 0 : E_ALL);
		error_reporting($er);
		...
		set_error_handler(function($errno, $errstr, $errfile, $errline, array $errcontext) {
			...
			FrontController::debug('warnings', array(array('message' => $message, 'context' => $errcontext)));
		...
		if (self::config('production')) ob_start(array('\Sarasa\Core\Template', "sanitize_output"));
		if (self::config('maintenance')) throw new CustomException('El sitio se encuentra en mantenimento', 500);
	...
	public final function debug($key, $value)
	{
		if (!isset(self::$debug)) self::$debug = array();
		if (isset(self::$debug[$key]) && is_array(self::$debug[$key]) && is_array($value)) self::$debug[$key] = array_merge(self::$debug[$key], $value);
		else self::$debug[$key] = $value;
	}

	private final static function createEntityManager()
	{
		$paths = array("../models");
	...
	public final static function entityManager() {
		if (!isset(self::$em)) self::createEntityManager();
		return self::$em;
	}

	public final static function config($value)
	{
	 ...
	 */
	public final static function route() {

		/** Chequeos previos **/
		$headers = apache_request_headers();
		if (isset($headers['AJAX_FUNCTION'])) $_SERVER['HTTP_AJAX_FUNCTION'] = $headers['AJAX_FUNCTION'];
		if (isset($headers['AJAX_URL'])) $_SERVER['HTTP_AJAX_URL'] = $headers['AJAX_URL'];
		...
		while(substr($aux, -1) == '/') $aux = substr($aux, 0, -1);
		...
		if (isset($auxget[1])) {
			...
			foreach ($auxvars as $auxvar) {
				...
				if(isset($auxvartemp[1])) $_VARIABLES[$auxvartemp[0]] = $auxvartemp[1];
			}
		}
		...
		$variables = explode ('/', $aux);
		if ($variables[0] == 'index.php') $variables = array_slice($variables, 1);
	
		$extension = '.php';
	
		$file = $_SERVER['DOCUMENT_ROOT'] . "/../routing.json";
		if (!is_file($file)) throw new CustomException('No se encontró el archivo de ruteo');
		$string = file_get_contents($file);
		$routing_var = json_decode($string, true);

		if (!is_array($routing_var)) throw new CustomException('El archivo de ruteo está corrupto o vacío');
		...
		while ($ruta = array_shift($routing_var)) {
			...
			if (!isset($_SERVER['HTTP_AJAX_FUNCTION']) && strtolower($ruta_method) != strtolower($_SERVER['REQUEST_METHOD'])) continue;
			if (isset($ruta['security']) && !isset($_SESSION[$ruta['security']])) continue;
			...
			while (substr($ruta['url'], -1) == '/') $ruta['url'] = substr($ruta['url'], 0, -1);
			while (substr($ruta['url'], 0, 1) == '/') $ruta['url'] = substr($ruta['url'], 1);
			...
			foreach ($url_partes as $parte) {
				if (strlen($parte) && ($parte{0} == ":" && $variables[$j])) $ruta_variables[substr($parte, 1)] = $variables[$j];
		...
		if (isset($url_final)) $url_final = '../'.$url_final;
	...
	public final static function handlePageException($e) {
