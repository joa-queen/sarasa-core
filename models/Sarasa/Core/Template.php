...
class Template extends \Smarty {
	...
	public function redirect($name, $parameters) {
		if (isset($parameters['params']) && count($parameters['params']) > 0) $parameters = $parameters['params'];
	...
	public static function getUrl($module, $parameters = array(), $method = 'get')
	{
		...
		while ($ruta = array_shift($routing_var)) {
			...
			if ($key == $module) {
				...
				if (!isset($_SERVER['SERVER_NAME'])) $_SERVER['SERVER_NAME'] = str_replace(array('http://', 'https://'), '', FrontController::config('domain'));
	...
	public function title($title, $postitle = true) {
		if ($postitle) $title .= ' | ' . FrontController::config('postitle');
	...
	public static function error500($e = null) {
		...
		if ($e && !FrontController::config('production')) $err = $e->getMessage();
